---
name: total-review-lite
description: 'Run both sonnet-review and gpt-review-lite on the current work, then merge their findings, dedupe them, and triage which issues are real vs overthinking. Outputs one clear, concise list of actual issues for the user to approve before fixing. Cheaper alternative to total-review (Fable 5 Max 1M + GPT 5.6 Sol Max) — uses Sonnet 5 + GPT 5.6 Medium instead. Use when the user says "/total-review-lite", "total review lite", "cheap review with both", or wants both reviewers at once without the max-tier cost. Differentiator: same merge/triage flow as total-review, but on cheaper models — for the max-tier version use total-review.'
---

# Total Review Lite

Run both code reviews on cheaper models, merge the findings, and give the user one shortlist of real issues to approve. Same flow as `total-review`, but swaps the max-tier reviewers for cheaper ones.

## Workflow

1. **Launch both reviewers in parallel** as two bb threads (default). Follow `/nagent`, `/bb-cli`, and each reviewer skill:
   - One `sonnet-review` worker (Sonnet 5).
   - One `gpt-review-lite` worker (GPT 5.6 Medium).
...
4. **Output to the user** — clear and very concise:
   ...
   - Mark which reviewer(s) found each: `[both]`, `[sonnet]`, or `[gpt]`. Issues found by both go first.
