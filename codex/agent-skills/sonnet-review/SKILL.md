---
name: sonnet-review
description: 'Launch a Sonnet 5 reviewer as a bb thread (default: /nagent) to do a deep, neutral senior-developer review of the current work and report its findings back verbatim. Cheaper alternative to fable-review (Fable 5 Max 1M). Use when the user says "/sonnet-review", "sonnet review", or asks for a cheaper Claude reviewer. Differentiator: reviewer model is Sonnet 5 — for the max-tier Claude reviewer use fable-review, for GPT use gpt-review or gpt-review-lite.'
---

# Sonnet Review

Launch a Sonnet 5 reviewer to review everything fully and carefully, as if it was a senior developer reviewing the work of a junior. This is the budget alternative to `fable-review` (which uses Fable 5 Max 1M) — same process, cheaper model.

**Default harness is bb.** Read `/nagent` and `/bb-cli` first. Spawn a bb thread with Claude Code **Sonnet 5** (look up the exact model id via the nagent lookup steps — do not guess; use the model's default reasoning effort unless the user says otherwise). Reuse this thread's environment so the reviewer sees the same files. Use `--parent-self` when this thread is coordinating the review. Then `bb thread wait` and show the exact `bb thread output`.
